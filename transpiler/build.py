...
basename = 'luccix/'+os.path.basename(os.path.dirname(os.path.realpath(f"{__file__}")))
out_dir = ""
changed = False
changedFiles = 0
# ANSI Color Codes
...
CONFIG = {
    "CFLAGS": ["-std=c11"],
    "CXXFLAGS": ["-std=c++20"],
    "CCXXFLAGS": ["-Wall", '-Wextra', "-Werror", "-fmax-errors=1", "-DCOMPILE", "-ggdb"]
}
...
def buildDir(directory_path: str, out_path: str, extra_args: list[str]=[]) -> int:
    ...
    for file in files:
        ...
        if not checkExtension(file, ["asm", "c", "lx", "cc", "cpp"]):
            continue
        code = callCmd(toCommand("cpp", extra_args+['-o', './tmp.txt', file, '-DCOMPILE']))
        if code != 0:
...
def main():
    ...
    print(f"[ ] Building transpiler...")
    if buildAndLinkDir("./src", out_dir, out_dir+'/transpiler.exe', ['-I./include', '-O2'], ['-O2']):
        print(f"[{ANSI_COLOR_BRIGHT_GREEN}X{ANSI_COLOR_RESET}] Building transpiler")
    else:
        print(f"[{ANSI_COLOR_RED}X{ANSI_COLOR_RESET}] Building transpiler")
